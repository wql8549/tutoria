...
def userList(req,id = 0):
    ...
    if page >= after_range_num and page<(count-befor_range_num):
...
def loginZabbix():
    ...
    auth_request = urllib2.request.Request(zabbix_url, auth_value)
    auth_request.add_header('Content-Type', 'application/json')
    auth_response = urllib2.request.urlopen(auth_request)
...
def dataHandle(*args,**kw):
    ...
    host_request = urllib2.request.Request(zabbix_url, value)
    host_request.add_header('Content-Type', 'application/json')
    response = urllib2.request.urlopen(host_request)
